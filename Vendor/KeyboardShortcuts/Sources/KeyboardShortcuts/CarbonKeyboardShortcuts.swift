...
enum CarbonKeyboardShortcuts {
    private(set) static var registrationErrors = [KeyboardShortcuts.Shortcut: OSStatus]()

    static func recordRegistrationResult(_ status: OSStatus, for shortcut: KeyboardShortcuts.Shortcut) {
        registrationErrors[shortcut] = status == noErr ? nil : status
        NotificationCenter.default.post(name: KeyboardShortcuts.registrationStatusDidChangeNotification, object: nil)
    }
	...
	static func register(
		...
		guard
			...
			recordRegistrationResult(registerError == noErr ? OSStatus(paramErr) : registerError, for: shortcut)
			return false
		}
		...
		recordRegistrationResult(noErr, for: shortcut)
		return true
	}
	...
	private static func softRegisterAll() {
		for hotKey in hotKeys.values {
			...
			guard
				...
				recordRegistrationResult(error == noErr ? OSStatus(paramErr) : error, for: hotKey.shortcut)
				hotKeys.removeValue(forKey: hotKey.carbonHotKeyId)
				continue
			}

			hotKey.carbonHotKey = eventHotKey
			recordRegistrationResult(noErr, for: hotKey.shortcut)
		}
	}
