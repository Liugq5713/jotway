...
public enum KeyboardShortcuts {
    public static let registrationStatusDidChangeNotification = Notification.Name("KeyboardShortcuts_registrationStatusDidChange")
    /// Posted after the saved binding and its registration attempt have both changed.
    public static let shortcutDidChangeNotification = Notification.Name.shortcutByNameDidChange

    /// A failed registration is distinct from a saved shortcut. Nil does not certify that a key press reaches the app.
    public static func registrationError(for name: Name) -> Int32? {
        guard let shortcut = getShortcut(for: name) else { return nil }
        return CarbonKeyboardShortcuts.registrationErrors[shortcut]
    }

    public static func isTakenBySystem(_ name: Name) -> Bool {
        getShortcut(for: name)?.isTakenBySystem ?? false
    }

    /// Read-only filtering, not a registration probe or a guarantee against other apps.
    @MainActor
    public static func isAvailableForRecommendation(_ shortcut: Shortcut) -> Bool {
        !shortcut.isDisallowed && !shortcut.isTakenBySystem
            && shortcut.takenByMainMenu == nil && CarbonKeyboardShortcuts.registrationErrors[shortcut] == nil
    }
	...
	*/
	...
	private static func register(_ shortcut: Shortcut) {
		guard !registeredShortcuts.contains(shortcut) || CarbonKeyboardShortcuts.registrationErrors[shortcut] != nil else {
			return
		}

		let succeeded = CarbonKeyboardShortcuts.register(
			shortcut,
			onKeyDown: handleOnKeyDown,
			onKeyUp: handleOnKeyUp
		)

		if succeeded { registeredShortcuts.insert(shortcut) }
	}
