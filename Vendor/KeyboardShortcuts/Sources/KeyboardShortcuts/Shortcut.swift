	...
	*/
	...
	func menuItemWithMatchingShortcut(in menu: NSMenu) -> NSMenuItem? {
		for item in menu.items {
			...
			// macOS 26 reports Command-Space on the visible emoji menu placeholder.
			// Its real shortcuts are on hidden siblings. Preserve explicit user overrides.
			if
				#available(macOS 26, *),
				item.action == #selector(NSApplication.orderFrontCharacterPalette(_:)),
				!item.isHidden,
				keyEquivalent == " ",
				keyEquivalentModifierMask == .command,
				item.userKeyEquivalent.isEmpty,
				menu.items.contains(where: {
					$0.action == item.action
						&& $0.isHidden
						&& $0.allowsKeyEquivalentWhenHidden
						&& $0.keyEquivalent == " "
						&& $0.keyEquivalentModifierMask == [.control, .command]
				})
			{
				continue
			}
