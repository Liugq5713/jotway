...
struct KeyboardShortcutsTests {
    @MainActor
    @Test("Recommendation filtering is read-only and rejects known registration and menu conflicts")
    func testRecommendationAvailabilityDoesNotRegisterOrChangePreferences() {
        ...
        defer { NSApp.mainMenu = previousMenu; CarbonKeyboardShortcuts.recordRegistrationResult(0, for: shortcut) }
        NSApp.mainMenu = NSMenu()
        CarbonKeyboardShortcuts.recordRegistrationResult(-9878, for: shortcut)
        #expect(!KeyboardShortcuts.isAvailableForRecommendation(shortcut))
        #expect(CarbonKeyboardShortcuts.registrationErrors[shortcut] == -9878)
        CarbonKeyboardShortcuts.recordRegistrationResult(0, for: shortcut)
        #expect(KeyboardShortcuts.isAvailableForRecommendation(shortcut) == !shortcut.isTakenBySystem)
        let item = NSMenuItem(title: "已有动作", action: nil, keyEquivalent: shortcut.nsMenuItemKeyEquivalent ?? "")
    ...
    @Test("Binding notification reports the final saved shortcut")
    func testBindingNotificationFollowsSavedPreference() {
        let name = KeyboardShortcuts.Name("notification-\(UUID().uuidString)")
        let shortcut = KeyboardShortcuts.Shortcut(.f18, modifiers: [.control, .option, .command])
        nonisolated(unsafe) var observed: KeyboardShortcuts.Shortcut?
        let observer = NotificationCenter.default.addObserver(forName: KeyboardShortcuts.shortcutDidChangeNotification,
            object: nil, queue: nil) { _ in observed = KeyboardShortcuts.getShortcut(for: name) }
        defer { NotificationCenter.default.removeObserver(observer); KeyboardShortcuts.setShortcut(nil, for: name) }
        KeyboardShortcuts.setShortcut(shortcut, for: name)
        #expect(observed == shortcut)
        KeyboardShortcuts.setShortcut(nil, for: name)
        #expect(observed == nil)
    }

    @Test("Registration errors are separate from saved bindings and clear on retry")
    func testRegistrationFailureAndRecovery() {
        let name = KeyboardShortcuts.Name("registrationFailureTest")
        let shortcut = KeyboardShortcuts.Shortcut(.f19, modifiers: [.control, .option, .command])
        KeyboardShortcuts.setShortcut(shortcut, for: name)
        defer { KeyboardShortcuts.setShortcut(nil, for: name) }
        CarbonKeyboardShortcuts.recordRegistrationResult(-9878, for: shortcut)
        #expect(KeyboardShortcuts.registrationError(for: name) == -9878)
        #expect(KeyboardShortcuts.getShortcut(for: name) == shortcut)
        CarbonKeyboardShortcuts.recordRegistrationResult(0, for: shortcut)
        #expect(KeyboardShortcuts.registrationError(for: name) == nil)
        KeyboardShortcuts.setShortcut(nil, for: name)
        #expect(KeyboardShortcuts.registrationError(for: name) == nil)
    }

    @Test("New defaults preserve existing shortcuts and explicit unbinding")
    func testDefaultChangePreservesStoredChoice() {
        let rawName = "defaultMigrationTest"
        let old = KeyboardShortcuts.Name(rawName, default: .init(.space, modifiers: [.command]))
        defer { UserDefaults.standard.removeObject(forKey: "KeyboardShortcuts_\(rawName)") }
        let updated = KeyboardShortcuts.Name(rawName, default: .init(.space, modifiers: [.command, .shift]))
        #expect(updated.shortcut == old.shortcut)
        #expect(updated.shortcut == .init(.space, modifiers: [.command]))
        KeyboardShortcuts.setShortcut(nil, for: updated)
        let reopened = KeyboardShortcuts.Name(rawName, default: .init(.space, modifiers: [.command, .shift]))
        #expect(reopened.shortcut == nil)
    }
	init() {
	...
	@Test("System emoji menu placeholder does not reserve Command-Space")
	@MainActor
	func testSystemEmojiMenuPlaceholder() {
		...
		// Match the visible placeholder and active hidden alternatives generated by macOS 26.
		let action = #selector(NSApplication.orderFrontCharacterPalette(_:))
		let placeholder = NSMenuItem(title: "表情与符号", action: action, keyEquivalent: " ")
		submenu.addItem(placeholder)
		let emoji = NSMenuItem(title: "表情与符号", action: action, keyEquivalent: " ")
		emoji.keyEquivalentModifierMask = [.control, .command]
		emoji.isHidden = true
		emoji.allowsKeyEquivalentWhenHidden = true
		submenu.addItem(emoji)
		let functionEmoji = NSMenuItem(title: "表情与符号", action: action, keyEquivalent: "e")
		functionEmoji.keyEquivalentModifierMask = .function
		functionEmoji.isHidden = true
		functionEmoji.allowsKeyEquivalentWhenHidden = true
		submenu.addItem(functionEmoji)

		let commandSpace = KeyboardShortcuts.Shortcut(.space, modifiers: .command)
		#expect(commandSpace.menuItemWithMatchingShortcut(in: menu) == nil)
		#expect(KeyboardShortcuts.Shortcut(.space, modifiers: [.control, .command]).menuItemWithMatchingShortcut(in: menu) === emoji)
		#expect(KeyboardShortcuts.Shortcut(.e, modifiers: .function).menuItemWithMatchingShortcut(in: menu) === functionEmoji)

		let otherCommand = NSMenuItem(title: "Another command", action: nil, keyEquivalent: " ")
		submenu.addItem(otherCommand)
		#expect(commandSpace.menuItemWithMatchingShortcut(in: menu) === otherCommand)
	}

	@Test("Explicit emoji menu shortcuts still reserve their keys")
	@MainActor
	func testExplicitEmojiMenuShortcut() {
		let menu = NSMenu()
		let emoji = NSMenuItem(title: "Emoji & Symbols", action: #selector(NSApplication.orderFrontCharacterPalette(_:)), keyEquivalent: " ")
		menu.addItem(emoji)

		#expect(KeyboardShortcuts.Shortcut(.space, modifiers: .command).menuItemWithMatchingShortcut(in: menu) === emoji)

		emoji.keyEquivalent = "e"
		emoji.keyEquivalentModifierMask = [.option, .command]
		#expect(KeyboardShortcuts.Shortcut(.e, modifiers: [.option, .command]).menuItemWithMatchingShortcut(in: menu) === emoji)
	}
