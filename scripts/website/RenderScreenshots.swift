        ...
        for dark in [false, true] {
            for empty in [true, false] {
                ...
                let content = EditorView(text: .constant(draft), focusTarget: EditorFocusTarget(), state: state, send: { _ in })
                    ...
                    .frame(width: LauncherMetrics.panelWidth, height: LauncherMetrics.inputCardMinHeight)
                let host = NSHostingView(rootView: content)
                let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 560, height: 88), styleMask: .borderless, backing: .buffered, defer: false)
                ...
                let bitmap = NSBitmapImageRep(bitmapDataPlanes: nil, pixelsWide: 1120, pixelsHigh: 176, bitsPerSample: 8, samplesPerPixel: 4, hasAlpha: true, isPlanar: false, colorSpaceName: .deviceRGB, bytesPerRow: 0, bitsPerPixel: 0)!
                bitmap.size = NSSize(width: 560, height: 88)
